...
import AppHeaderCourriersortant from "../../components/app/AppHeaderCourriersortant";
...
/**
 * Screen pour faire la liste des courries sortants
...
export default function AllCourrierSortantScreen() {
    ...
    const onLoadMore = async () => {
        try {
            ...
            const crs = await getCourrierSortant(newOffset)
            setOffset(newOffset)
    ...
    const getCourrierSortant = async (offset = 0, statut) => {
        try {

            var url = `/courrier/courrier_entrants/courriersortant?limit=${LIMIT}&offset=${offset}`
            
            if (statut) {
                setLoading(true)
                url = `/courrier/courrier_entrants/courriersortant?ETAPE_ID=${statut}&limit=${LIMIT}&offset=${offset}`
    ...
    useFocusEffect(useCallback(() => {
        (async () => {
            try {
                setOffset(0)
                const response = await getCourrierSortant(0, idstatut)
    ...
    */
                    ...
                    <View style={styles.selectContainer}>
                        ...
                        <View style={{}}>
                            ...
                            <View>
                                ...
                                >
                                    <Text >
                                        {idstatut == 4 ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#18678E" /> :
                                            <MaterialCommunityIcons name="radiobox-blank" size={24} color="#777" />} En cours</Text>
                                </TouchableOpacity>
                                ...
                                >
                                    <Text>
                                        {idstatut == 2 ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#18678E" /> :
                                            <MaterialCommunityIcons name="radiobox-blank" size={24} color="#777" />} Transmis </Text>
                                </TouchableOpacity>
                            </View>
                            <View >
                                <TouchableOpacity
                                ...
                                >
                                    <Text >
                                        {idstatut == 3 ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#18678E" /> :
                                            <MaterialCommunityIcons name="radiobox-blank" size={24} color="#777" />} En attente du remettant</Text>
                                </TouchableOpacity>
                                ...
                                >
                                    <Text>
                                        {idstatut == 1 ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#18678E" /> :
                                            <MaterialCommunityIcons name="radiobox-blank" size={24} color="#777" />} En attente de transmission </Text>
                                </TouchableOpacity>
    ...
    return (
        <>
            ...
            <AppHeaderCourriersortant notification={nbres.NBRE} modal={filterModalizeRef}/>
            {loading ? <View style={styles.emptyContaier}>
                <ActivityIndicator animating size={'large'} color={'#777'} />
            </View> :
                courriers.length <= 0 ? <View style={styles.emptyContaier}>
                    <Image source={require('../../../assets/images/send-mail.png')} style={styles.emptyImage} />
                    <Text style={styles.emptyTitle}>
                        Aucun courrier trouvé
                    </Text>
                    <Text style={styles.emptyDesc}>
                        Aucun courrier Transmis
                    </Text>
                </View> :
                    <FlatList
                                    ...
                                    <View style={styles.header}>
                                        <View style={styles.cardEntete}>
                                            <View style={styles.icon}>
                                                <Image source={require("../../../assets/images/send-mail-small.png")} style={styles.iconImage} />
                                            </View>
                                            <View style={styles.cardDescription}>
                                                <Text style={styles.titleObject} numberOfLines={2}>{courrier.OBJET_LETTRE}</Text>
                                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    ...
                                                    <Text style={styles.titleDate}>{moment(courrier.DATE_ENVOIE).format("DD-MM-YYYY")}</Text>
                                                </View>
                    ...
                    />}
            <FloatingAction
                actions={user.ID_PROFIL != 3 ? actions : null}
                onPressItem={name => {
