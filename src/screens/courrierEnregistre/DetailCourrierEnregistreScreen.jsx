...
/**
 * Screen pour faire le detail du courrier enregistre
 * @author NDAYISBA Claudine <claudine@mediabox.bi>
 * @date 09/06/2023 à 15:40
 * @returns 
 */
export default function DetailCourrierEnregistreScreen() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const response = await fetchApi(`/courrier/courrier_entrants/courrierenregistre/${id}`)
    ...
    const classer = () => {
        navigation.navigate('ClasseurScreen', {
          ...route.params,
          courrier: detail
        })
      }
 ...
 */
    ...
    return (
        ...
        <>
                ...
                <ScrollView style={{ backgroundColor: "#E1EAF3", paddingHorizontal: 10 }}>
                    ...
                    <View style={[styles.detailCard, { marginBottom: 10 }]}>
                        ...
                        {detail.affected.length > 0 ? <View style={styles.detail}>
                            ...
                            {detail.affected[0].DESCRIPTION?<Text style={styles.detailTitle}>Services {detail.affected[0].DESCRIPTION}</Text>:null}
                            {detail.affected.map((affecte, index) => {
                                return (
                                    <View key={index} style={{}}>
                                       {(affecte.NOM && affecte.PRENOM)?  <Text style={[styles.detailValue]} >- {affecte.NOM} {affecte.PRENOM}</Text>   :null}
                                    </View>
                                )
                            })}
            ...
            {detail.STATUT_TRAITEMENT != 1 && !detail.NUMERO_CLASSEUR ? <View style={styles.courrierFooter}>
                <TouchableNativeFeedback onPress={classer}>
