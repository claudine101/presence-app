...
import AppHeaderEnregistre from "../../components/app/AppHeaderEnregistre";
/**
 * Screen pour faire la liste des courries enregistre par  secretaite
 * @author NDAYISABA claudine<claudine@mediabox.bi>
 * @date 09/10/2023 à 15:37
 * @returns 
 */
export default function AllCourrierEnregistreScreen() {
    ...
    const getCourrierEntrant = async (offset = 0, statut) => {
        try {
            ...
            var url = `/courrier/courrier_entrants/courrierenregistre?limit=${LIMIT}&offset=${offset}`
            if (statut != null) {
                setLoading(true)

                url = `/courrier/courrier_entrants/courrierenregistre?STATUT_TRAITEMENT=${statut}&limit=${LIMIT}&offset=${offset}`
            }
    ...
    */
    ...
    return (
        <>
            ...
            <AppHeaderEnregistre notification={nbres.NBRE} modal={filterModalizeRef}/>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item: courrier, index }) => {
                            return (
                                    ...
                                    <TouchableNativeFeedback onPress={() => navigation.navigate('DetailCourrierEnregistreScreen', { courrier: courrier })}>
                                        {loading ? <View style={styles.emptyContaier}>
                                            <ActivityIndicator animating size={'large'} color={'#777'} />
                                        </View> :
