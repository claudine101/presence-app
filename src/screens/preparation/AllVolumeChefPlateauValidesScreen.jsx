 ...
 * @returns 
 */
export default function AllVolumeChefPlateauValidesScreen() {
    ...
    useFocusEffect(useCallback(() => {
        (async () => {
            try {
                setLoading(true)
                const vol = await fetchApi(`/preparation/volume/volumePrepares `)
                setAllVolumes(vol.result.data)
                console.log(vol.result)
    ...
    useFocusEffect(useCallback(() => {
        setNextRouteName('ChefPlatauValideDetailsScreen')
    }, [user]))
    return (
        <>
