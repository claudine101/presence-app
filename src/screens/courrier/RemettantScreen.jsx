...
const ListHeader = ({ handleRemettantPress, selectedExpediteur, selectedRemettant, newRemettant, returnkey, newexpediteur }) => {
    ...
    return (
        <View style={styles.listHeader}>
            ...
            <TouchableNativeFeedback onPress={() => navigation.navigate("NewRemettantScreen", { newRemettant, returnkey, newexpediteur, ...route.params })}>
                            ...
                            {returnkey == "expediteur" ?
                                <>
                                    ...
                                    <Text style={styles.institutionAddress} numberOfLines={1} >
                                        Enregistrer un nouveau expediteur
                                    </Text>
                                </> :
                                <>
                                    <Text style={styles.institutionName} numberOfLines={1} >
...
export default function RemettantScreen() {
    const navigation = useNavigation()
    const route = useRoute()
    ...
    const handleRemettantPress = remets => {
        if (inEdit) {
            ...
            navigation.navigate("UpdateCourrierScreen", { ...route.params, [returnkey]: remets })
        }
        else {
            setSelectedRemettant(remets)
            navigation.navigate("CourrierScreen", { [returnkey]: remets })
        }
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <View style={styles.exiitSearch}>
                    ...
                    {!isInSearch ? <Text style={styles.title}>{returnkey == "expediteur" ? "Sélectionner l'expediteur" : 'Sélectionner le remettant'}</Text> : null}
