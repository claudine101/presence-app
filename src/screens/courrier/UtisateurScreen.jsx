    ...
    return (
        <TouchableNativeFeedback onPress={() => handleUtilPress(remet)}>
            ...
            <View style={styles.institution}>
                ...
                {isSelected(remet.USERS_ID) ? <MaterialCommunityIcons name="radiobox-marked" size={24} color="#18678E" /> :
...
export default function UtisateurScreen() {
    const navigation = useNavigation()
    const route = useRoute()
    const [loading, setLoading] = useState(true)
    const [remets, setRemets] = useState([])
    const [isInSearch, setIsInSearch] = useState(false)
    const { utilis, newRemettant, selectedUsers, courrier, selectedAffect, inAffect, inEdit } = route.params
    ...
    const enregistrement = async () => {
        if (inAffect) {
            ...
            navigation.navigate("CourrierScreen", { selectedUser })
        }
        ...
        (async () => {
            if (q && q.trim() != '') {
                try {
                    setLoading(true)
                    if (inAffect) {
                        const remt = await fetchApi(`/services/utilis?q=${courrier.ID_COURRIER_ENTRANT}&reserch=${q}`)
                        setRemets(remt.result)
                    }
                    else {
                        const remt = await fetchApi(`/services/utilis?q=${0}&reserch=${q}`)
                        setRemets(remt.result)
                    }
                } catch (error) {
                    console.log(error)
                ...
                try {
                    setLoading(true)
                    if (inAffect) {
                        const remt = await fetchApi(`/services/utilis?q=${courrier.ID_COURRIER_ENTRANT}`)
                        setRemets(remt.result)
                    }
                    else {
                        const remt = await fetchApi(`/services/utilis?q=${0}`)
                        setRemets(remt.result)
                    }
                } catch (error) {
                    console.log(error)
    ...
    useFocusEffect(useCallback(() => {
        (async () => {
            try {
                if (inAffect) {
                const remt = await fetchApi(`/services/utilis?q=${courrier ? courrier.ID_COURRIER_ENTRANT : 0} `)
                setRemets(remt.result) 
            }
            else{
                const remt = await fetchApi(`/services/utilis?q=${0} && reserch=${q}` )
